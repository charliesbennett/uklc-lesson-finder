    ...
    if (file) {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const lessonsToImport = importedLessons.map(({ id, created_at, ...lesson }) => ({
            ...lesson,
            created_at: new Date().toISOString()
          }));
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLessons.map(lesson => (
            <div key={lesson.id} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition" style={{ borderTop: '3px solid #ec273b' }}>
              ...
              {lesson.description && (
                <p className="text-sm mb-3 whitespace-pre-line" style={{ color: '#1c3048', opacity: 0.8 }}>{lesson.description}</p>
              )}
