...
export default function UKLCLessonManager() {
  ...
  const commonTags = [
    ...
    'A1', 'A2', 'B1', 'B2', 'C1', 'C2',
    // Focus Areas
    'Language Focus', 'Leadership Focus', 'Culture Focus',
    // Subject Areas
    'Food', 'Art', 'Science', 'Maths', 'History', 'Culture', 'Technology', 'Literature', 'Psychology', 'AI', 'Sociology', 'Digital Literacy', 'Mindfulness', 'Geography', 'Movies/TV', 'Sports', 'Video Games', 'Photography', 'Fashion', 'Music',
    // Skills
    'Vocabulary', 'Grammar', 'Listening', 'Reading', 'Writing', 'Speaking', 'Debate', 'Presentation Design',
    // Activity Types
    'Group Work', 'Outdoor Lesson', 'Surveys', 'Comparatives',
    // Themes
    'Sustainability', 'Travel', 'Weather'
  ];
  ...
  const loadLessons = async () => {
    try {
      // Try window.storage first (Claude artifact storage)
      if (window.storage) {
      ...
      const localData = localStorage.getItem('uklc-lessons');
      if (localData) {
        setLessons(JSON.parse(localData));
      }
    } catch (error) {
  ...
  const saveLessons = async (updatedLessons) => {
    try {
      ...
      if (window.storage) {
        ...
        // Fallback to localStorage
        localStorage.setItem('uklc-lessons', JSON.stringify(updatedLessons));
      }
      ...
      try {
        ...
        alert('Failed to save lessons. Please check your browser settings:\n\n1. Make sure cookies/storage are enabled\n2. Try using a regular browser window (not private/incognito)\n3. Clear some browser data if storage is full');
        throw localError;
      }
    }
  };
  ...
  const addLesson = (lesson) => {
    const newLesson = {
      ...
      title: lesson.title || `${lesson.week} - ${lesson.programme} - ${lesson.level}`,
  ...
  const filteredLessons = lessons.filter(lesson => {
    ...
    return matchesSearch && matchesWeek && matchesProgramme && matchesLevel && matchesTags;
  });
...
function LessonModal({ lesson, weeks, programmes, levels, commonTags, onSave, onClose }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.week || !formData.programme || !formData.level) {
      alert('Please fill in Week, Programme, and Level');
      return;
    }
    if (uploadMethod === 'sharepoint' && urlError) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border-2 rounded-lg focus:outline-none"
                style={{ borderColor: '#e6eef3' }}
                placeholder="Leave blank to auto-generate (e.g., Week A - Connection - Level 1)"
              />
              <p className="mt-1 text-xs" style={{ color: '#1c3048', opacity: 0.6 }}>
                💡 If left blank, title will be: {formData.week || 'Week'} - {formData.programme || 'Programme'} - {formData.level || 'Level'}
              </p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                style={{ borderColor: '#e6eef3' }}
                rows="3"
                placeholder="E.g., Topics covered: Food & restaurants, weather, daily routines..."
              />
              <p className="mt-1 text-xs" style={{ color: '#1c3048', opacity: 0.6 }}>
                💡 Add searchable keywords about what's in this PDF to help teachers find it
              </p>
            </div>
